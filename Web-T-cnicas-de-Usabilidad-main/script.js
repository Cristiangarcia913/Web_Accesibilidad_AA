...
function navigateTo(view, payload = null) {
    ...
    if (view === 'inicio') {
        ...
        // Restaurar el foco al botón de la tarjeta si volvemos desde el detalle de esa técnica
        if (state.currentTechnique) {
            const cardBtn = document.getElementById(`card-${state.currentTechnique}`);
            if (cardBtn) {
                cardBtn.focus();
            }
            state.currentTechnique = null;
...
function renderGrid() {
    ...
    grid.innerHTML = filteredData.map(t => `
        <button type="button" class="technique-card" id="card-${t.id}" onclick="navigateTo('detalle', '${t.id}')">
            <h3>${t.name}</h3>
...
function renderDetail() {
    ...
    detTitle.textContent = tech.name;
    detTitle.setAttribute('tabindex', '-1');
    ...
    // Gestión activa del foco para accesibilidad en SPA (WCAG 2.1 AA)
    detTitle.focus();
}
